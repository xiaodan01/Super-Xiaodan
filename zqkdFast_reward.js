...
async function GetCoinInfo(uIdx) {
    ...
    let url = `https://user.youth.cn/v1/user/userinfo.json?is_add_desktop=1&${userCk}&app_version=2.5.5&channel=c6004&device_model=RedmiNote5&device_brand=XIAOMI&resolution=1080*2160&os_version=29&is_wxaccount=1&active_channel=c6004&access=wifi`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
