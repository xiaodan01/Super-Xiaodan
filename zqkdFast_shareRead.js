...
async function ReadArts(uIdx,sign) {
    ...
    let uid = userCk.match(/uid=(\w+)/)[1]
    let url = `https://user.youth.cn/v1/article/detail.json?signature=${sign}&source=articleDetail&${userCk}&app_version=2.5.5&channel=c6004&device_model=RedmiNote5&device_brand=XIAOMI&resolution=1080*2160&os_version=29&is_wxaccount=1&active_channel=c6004&access=wifi`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function ShareArticleCallback(uIdx) {
    ...
    let userCk = userCookieArr[userIdx]
    let uid = userCk.match(/uid=(\w+)/)[1]
    let url = `https://user.youth.cn/FastApi/ArticleTop/shareArticleCallback.json?${userCk}&app_version=2.5.5&channel=c6004&device_model=RedmiNote5&device_brand=XIAOMI&resolution=1080*2160&os_version=29&is_wxaccount=1&active_channel=c6004&access=wifi&from=1`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function ShareEnd(uIdx,artId) {
    ...
    let userCk = userCookieArr[userIdx]
    let uid = userCk.match(/uid=(\w+)/)[1]
    let url = `https://user.youth.cn/FastApi/article/shareEnd.json?${userCk}&app_version=2.5.5&channel=c6004&device_model=RedmiNote5&device_brand=XIAOMI&resolution=1080*2160&os_version=29&is_wxaccount=1&active_channel=c6004&access=wifi&from=1&device_platform=android&article_id=${artId}&stype=WEIXIN`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
function populateGetUrl(url){
    let urlObject = {
        url: url,
        headers: {
            'User-Agent' : 'Mozilla/5.0 (Linux; Android 10; Redmi Note 5 Build/QKQ1.190910.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 hap/1.9/xiaomi com.miui.hybrid/1.9.0.4 com.youth.kandianquickapp/2.5.5 ({"packageName":"com.android.browser","type":"url","extra":{}})',
